...
import OnlineWallet.controller.WalletController;
...
class WalletControllerTest {
	...
	void whenDeposit1000ThenResponse200() {
		...
		operationDeposit.setWalletId(walletId1);
	...
	void whenDepositMinus1000ThenResponse400() {
		OperationDTO operationDeposit = new OperationDTO();
		operationDeposit.setWalletId(walletId2);
		operationDeposit.setOperationType(OperationType.DEPOSIT);
		operationDeposit.setAmount(BigDecimal.valueOf(-1000));
		ResponseEntity<?> response = restTemplate.exchange(
				updateWalletUri,
	...
	void whenWalletUUIDNotFoundThenResponse404() {
		...
		operationDeposit.setWalletId(subWalletId);
		operationDeposit.setOperationType(OperationType.DEPOSIT);
		operationDeposit.setAmount(BigDecimal.valueOf(1000));
		ResponseEntity<?> response = restTemplate.exchange(
				updateWalletUri,
	...
	void whenWithdrawMoreBalanceThenResponse403() {
		OperationDTO operationDeposit = new OperationDTO();
		operationDeposit.setWalletId(walletId3);
		operationDeposit.setOperationType(OperationType.WITHDRAW);
		...
		Runnable depositTask = () -> {
			try {
				for (int i = 0; i < depositsPerThread; i++) {
					OperationDTO operationDeposit = new OperationDTO();
					operationDeposit.setWalletId(walletId4);
					operationDeposit.setOperationType(OperationType.DEPOSIT);
		...
		Runnable depositTask = () -> {
			try {
				for (int i = 0; i < withdrawPerThread; i++) {
					OperationDTO operationDeposit = new OperationDTO();
					operationDeposit.setWalletId(walletId5);
					operationDeposit.setOperationType(OperationType.WITHDRAW);
